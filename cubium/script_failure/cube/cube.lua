...
local cube_interface =
{
  get_created_items = function()
    return storage.created_items
  end,
  set_created_items = function(map)
    storage.created_items = map or error("Remote call parameter to cube set created items can't be nil.")
  end,
  get_respawn_items = function()
    return storage.respawn_items
  end,
  set_respawn_items = function(map)
    storage.respawn_items = map or error("Remote call parameter to cube set respawn items can't be nil.")
  end,
  ...
  set_chart_distance = function(value)
    storage.chart_distance = tonumber(value) or error("Remote call parameter to cube set chart distance must be a number")
  end,
  ...
  set_ship_items = function(map)
    storage.crashed_ship_items = map or error("Remote call parameter to cube set created items can't be nil.")
  end,
  get_debris_items = function()
    return storage.crashed_debris_items
  end,
  set_debris_items = function(map)
    storage.crashed_debris_items = map or error("Remote call parameter to cube set respawn items can't be nil.")
  end,
  get_ship_parts = function()
    return storage.crashed_ship_parts
  end,
  set_ship_parts = function(parts)
    storage.crashed_ship_parts = parts or error("Remote call parameter to cube set ship parts can't be nil.")
  end
}

if not remote.interfaces["cube"] then
  remote.add_interface("cube", cube_interface)
end

local is_debug = function()
  local surface = nil
  
  if(game.surfaces.cubium == nil) then
    surface = game.planets["cubium"].create_surface()
    surface.request_to_generate_chunks({0, 0}, 3)
    surface.force_generate_chunk_requests()
  else
    surface = game.surfaces.cubium
  end
  game.player.teleport(surface.find_non_colliding_position("character", {0, 0}, 0, 1))
  game.player.force.set_spawn_position({0,0}, surface) 
...
cube.on_configuration_changed = function()
...
cube.on_init = function()
  ...
  for k,v in pairs(forces) do
    if (v.add_research("planet-discovery-cubium")) then
      v.research_progress = 1
    end
